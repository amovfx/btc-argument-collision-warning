...
cd $BITCOIN_GIT_DIR
git checkout argument-collision-warning
configure #these are my functions, you can just run ./configure
build #these are my functions, you can just run make -j N

cd $BITCOIN_BUILD_DIR
CHAIN_MERGE_TEST_OUT=/tmp/argument-collision-warning_results.txt ${BITCOIN_BUILD_DIR}/src/test/test_bitcoin --run_test=util_tests/util_ChainMerge
export BRANCH_CHAIN_MERGE_TEST_OUT=/tmp/argument-collision-warning_results.txt
